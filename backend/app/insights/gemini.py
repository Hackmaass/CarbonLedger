...
# Prompt for extracting footprint data from images (bills, receipts).
_AUDIT_SYSTEM_INSTRUCTION = (
    "You are a data extraction expert. Given an image of a utility bill (electricity, "
    "gas) or a receipt, extract relevant carbon footprint data. Focus on: monthly "
    "kWh for electricity or gas, and spending on goods. If the document is an "
    "electricity bill, extract the total kWh used. If it's a gas bill, extract kWh or "
    "therms. If it's a grocery receipt, extract the total amount. Return valid JSON "
    "matching the schema."
)
    ...
    "properties": {
        "home": {
            ...
            "properties": {
                ...
                "natural_gas_kwh_per_month": {"type": "number"},
            },
        },
        "consumption": {
            "type": "object",
            "properties": {
                "goods_spend_usd_per_month": {"type": "number"},
            },
        },
    },
}
...
def extract_from_image(image_bytes: bytes, mime_type: str, settings: Settings) -> dict:
    """Extract footprint data from an image using Gemini Flash 2.0."""
    from google import genai
    from google.genai import types
    ...
    response = client.models.generate_content(
        ...
        contents=[
            ...
            "Extract the utility/consumption data from this image.",
        ],
        config=types.GenerateContentConfig(
