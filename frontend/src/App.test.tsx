...
describe("App", () => {
  ...
  it("extracts data via AI Auditor when a file is uploaded", async () => {
    vi.mocked(api.extractDataFromImage).mockResolvedValue({
      home: { electricity_kwh_per_month: 450, natural_gas_kwh_per_month: 0, household_size: 1 },
    });
    await renderApp();

    const file = new File(["foo"], "bill.png", { type: "image/png" });
    const input = screen.getByLabelText(/ai audit/i);
    await userEvent.upload(input, file);

    await waitFor(() => expect(api.extractDataFromImage).toHaveBeenCalledWith(file));
    // Verify that the field was auto-filled (NumberField for electricity)
    expect(screen.getByLabelText(/electricity per month/i)).toHaveValue(450);
  });
